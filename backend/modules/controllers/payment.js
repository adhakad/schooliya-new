...
let ValidatePayment = async (req, res) => {
  ...
  try {
    ...
    const thirtyOneDaysInMillis =    15 * 60 * 1000; //31 * 24 * 60 * 60 * 1000;
    ...
    if (existingAdminPlan) {
      ...
      if (currentTime >= oneMonthBeforeExpiration) {
        expirationDate = new Date(Date.now() + 15 * 60 * 1000); //365 * 24 * 60 * 60 * 1000
      } else {
        const remainingTime = currentExpirationDate - currentTime;
        expirationDate = new Date(Date.now() + 15 * 60 * 1000 + remainingTime);
      }
    } else {
